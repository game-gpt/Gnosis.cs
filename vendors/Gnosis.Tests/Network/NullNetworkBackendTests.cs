...
public class NullNetworkBackendTests : GnosisTester
{
    ...
    public void Send_不抛出异常()
    {
        ...
        Assert.DoesNotThrow(() => _backend.Send([1, 2, 3]));
    }
    ...
    public void SendReliable_不抛出异常()
    {
        ...
        Assert.DoesNotThrow(() => _backend.SendReliable([1, 2, 3]));
    }
    ...
    {
        AssertThrows<InvalidOperationException>(() => _backend.Send([1]));
    }

    [Test]
    public void SendReliable_未连接时抛出InvalidOperationException()
    {
        AssertThrows<InvalidOperationException>(() => _backend.SendReliable([1]));
    }
