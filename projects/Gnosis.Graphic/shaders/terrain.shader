using gg_shader::f32::{vec2, vec3, vec4, mat4}

shader terrain {
    uniform mvp: mat4,
    uniform model: mat4,

    varying v_normal: vec3,
    varying v_uv: vec2,
    varying v_world_pos: vec3,

    [vertex_main]
    vs_main(position: vec3, normal: vec3, uv: vec2) -> vec4 {
        v_normal = normalize((model * vec4(normal, 0.0)).xyz)
        v_uv = uv
        v_world_pos = (model * vec4(position, 1.0)).xyz
        return mvp * vec4(position, 1.0)
    }

    [fragment_main]
    fs_main() -> vec4 {
        let normal = normalize(v_normal)
        let height = v_world_pos.y

        let rock_color = vec3(0.4, 0.35, 0.3)
        let grass_color = vec3(0.2, 0.5, 0.15)
        let snow_color = vec3(0.9, 0.9, 0.95)
        let sand_color = vec3(0.76, 0.7, 0.5)

        let slope = 1.0 - max(dot(normal, vec3(0.0, 1.0, 0.0)), 0.0)

        let mut base_color: vec3
        if height < 2.0 {
            base_color = sand_color
        } else if height < 30.0 {
            let t = (height - 2.0) / 28.0
            base_color = mix(sand_color, grass_color, t)
        } else if height < 60.0 {
            let t = (height - 30.0) / 30.0
            base_color = mix(grass_color, rock_color, t)
        } else {
            let t = min((height - 60.0) / 20.0, 1.0)
            base_color = mix(rock_color, snow_color, t)
        }

        if slope > 0.5 {
            let t = min((slope - 0.5) / 0.3, 1.0)
            base_color = mix(base_color, rock_color, t)
        }

        let light_dir = normalize(vec3(0.5, 1.0, 0.3))
        let ambient = 0.3
        let diffuse = max(dot(normal, light_dir), 0.0) * 0.7
        let lighting = ambient + diffuse

        return vec4(base_color * lighting, 1.0)
    }
}
