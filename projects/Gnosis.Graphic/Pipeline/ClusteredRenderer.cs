...
public sealed class ClusteredRenderer
{
    #region 字段

    private readonly IDevice _device;
    private readonly RenderPipeline _pipeline;
    private readonly LightCluster _lightCluster;
    ...
    public LightCluster LightCluster => _lightCluster;

    #endregion

    #region 构造函数

    public ClusteredRenderer(IDevice device, float near = 0.1f, float far = 1000.0f)
    {
        _device = device;
        _pipeline = new RenderPipeline("ClusteredRenderer");
        _lightCluster = new LightCluster(near, far);
    }
    ...
    public void Initialize(nint windowHandle, uint width, uint height, float fov)
    {
        ...
        CreateSyncObjects();
        _lightCluster.BuildClusterGrid(width, height, fov);
    }

    public void Resize(uint width, uint height, float fov)
    {
        _device.WaitIdle();
    ...
    public void Render(RenderContext context, ReadOnlySpan<ClusterLightInfo> lights)
    {
        ...
        _lightCluster.AssignLightsToClusters(lights, context.View.ViewMatrix);
