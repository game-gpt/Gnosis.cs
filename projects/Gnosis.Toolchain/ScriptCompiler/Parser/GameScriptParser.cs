...
public class GameScriptParser : IParser
{
    ...
    private ComponentDecl ParseComponentDecl(IReadOnlyList<AttributeDecl>? attrs = null)
    {
        ...
        return new ComponentDecl(name, attrs ?? [], fields, Span: startToken.ToSourceSpan());
    }
    ...
    private SystemDecl ParseSystemDecl(IReadOnlyList<AttributeDecl>? attrs = null)
    {
        ...
        while (!Check(TokenType.Delimiter, "}") && !IsAtEnd())
        {
            if (Check(TokenType.Identifier, "query") ||
                (Check(TokenType.Identifier) && Peek().Value.StartsWith("query_")))
            {
        ...
        return new SystemDecl(name, attrs ?? [], queries, methods, Span: startToken.ToSourceSpan());
    }
    ...
    private QueryExpr ParseQueryExpr()
    {
        ...
        return new QueryExpr(kind, componentTypes, filters, Span: startToken.ToSourceSpan());
    }
    ...
    private WidgetDecl ParseWidgetDecl()
    {
        ...
        return new WidgetDecl(name, properties, renderMethod, Span: startToken.ToSourceSpan());
    }
    ...
    private PluginDecl ParsePluginDecl()
    {
        ...
        return new PluginDecl(name, requiresArch, providesMacros, providesCapabilities, functions, Span: startToken.ToSourceSpan());
    }
    ...
    private AstNode ParsePrimary()
    {
        ...
        if (Check(TokenType.Identifier, "new"))
        {
            ...
            return new TermCallExpression(new IdentifierNode($"new_{typeName.Name}"), args);
        }
