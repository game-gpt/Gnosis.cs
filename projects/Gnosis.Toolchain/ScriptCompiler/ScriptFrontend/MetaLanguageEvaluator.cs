...
public partial class MetaLanguageEvaluator : IMetaLanguageEvaluator
{
    ...
    private AstNode EvaluateForStmt(ForStmt stmt)
    {
        ...
        return new ForStmt(initializer, condition, update, body, Span: stmt.Span);
    }
    ...
    private AstNode EvaluateMetaExpression(string content)
    {
        ...
        {
            return new LiteralExpr(LiteralType.String, strResult);
        }

        if (result is int intResult)
        {
            return new LiteralExpr(LiteralType.Number, intResult.ToString());
        }

        if (result is float floatResult)
        {
            return new LiteralExpr(LiteralType.Number, floatResult.ToString());
        }

        if (result is bool boolResult)
        {
            return new LiteralExpr(LiteralType.Boolean, boolResult ? "true" : "false");
        }

        return new LiteralExpr(LiteralType.String, result?.ToString() ?? "");
    }
    ...
    private AstNode EvaluateMetaCodeBlock(string content)
    {
        ...
        {
            return new BlockStmt(unit.Declarations);
        }
