...
public class NetworkManagerTests : GnosisTester
{
    ...
    {
        AssertThrows<InvalidOperationException>(() => _manager.SendToServer([1]));
    }

    [Test]
    public void SendToAll_未初始化时抛出InvalidOperationException()
    {
        AssertThrows<InvalidOperationException>(() => _manager.SendToAll([1]));
    }
    ...
    public void SendToServer_已连接时不抛出异常()
    {
        ...
        Assert.DoesNotThrow(() => _manager.SendToServer([1, 2, 3]));
    }
    ...
    public void SendToAll_已连接时不抛出异常()
    {
        ...
        Assert.DoesNotThrow(() => _manager.SendToAll([1, 2, 3]));
    }
