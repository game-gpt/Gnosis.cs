...
/// <summary>
///     扩散模型着色器后端，将 Shader IR 转换为 SafeTensors 格式的模型权重。
/// </summary>
/// <remarks>
...
public sealed class DiffusionShaderBackend : IShaderBackend
{
    ...
    /// <summary>
    ///     编译 Shader IR 为 SafeTensors 格式的扩散模型权重。
    /// </summary>
        ...
        foreach (var instruction in function.Instructions)
        {
            switch (instruction.OpCode)
            {
                case ShaderIrOpCode.Mul:
                    AddWeight("noise_weight", SafeTensorDType.Float32, [1024, 1024]);
                    break;

                case ShaderIrOpCode.Add:
                    AddWeight("noise_bias", SafeTensorDType.Float32, [1024]);
                    break;

                case ShaderIrOpCode.Sub:
                    AddWeight("denoise_weight", SafeTensorDType.Float32, [1024, 1024]);
                    break;
